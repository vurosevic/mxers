 ...
 * @author Vladimir
 */
public class LazyDataModelAukcijaCena extends LazyDataModel<AukcijaCena>{
    
    private List<AukcijaCena> datasource;

    public LazyDataModelAukcijaCena(List<AukcijaCena> datasource) {
        this.datasource = datasource;
    }

    @Override
    public AukcijaCena getRowData(String rowKey) {
        for(AukcijaCena gz : datasource) {
    ...
    @Override
    public Object getRowKey(AukcijaCena object) {
        return object.getId();
    }    
        
    @Override
    public List<AukcijaCena> load(int first, int pageSize, String sortField, SortOrder sortOrder, Map<String, Object> filters) {        
        List<AukcijaCena> data = new ArrayList<AukcijaCena>();
   //filter
        for(AukcijaCena ps : datasource) {
            boolean match = true;
                    ...
                    try {
                        ...
                        Object filterValue = filters.get(filterProperty);
                           ...
                           if (filterProperty.indexOf('.') > 0) {
                            ...
                            field = AukcijaCena.class.getDeclaredField(filterProperty);                                
                            ...
                            if (field.getType()==java.util.Date.class){
                               ...
                               fieldValue = formatter.format(tmp);
                                ...
                                field2 = Smer.class.getDeclaredField(filterProperty2);      
                                field2.setAccessible(true);
                                fieldValue = String.valueOf(field2.get(ps.getSmer()));  
