 ...
 * @author Vladimir
 */
public class LazyDataModelAukcija extends LazyDataModel<Aukcija>{
    
    private List<Aukcija> datasource;

    public LazyDataModelAukcija(List<Aukcija> datasource) {
        this.datasource = datasource;
    }

    @Override
    public Aukcija getRowData(String rowKey) {
        for(Aukcija gz : datasource) {
    ...
    @Override
    public Object getRowKey(Aukcija object) {
        return object.getId();
    }    
        
    @Override
    public List<Aukcija> load(int first, int pageSize, String sortField, SortOrder sortOrder, Map<String, Object> filters) {        
        List<Aukcija> data = new ArrayList<Aukcija>();
   //filter
        for(Aukcija ps : datasource) {
            boolean match = true;
                ...
                for (Iterator<String> it = filters.keySet().iterator(); it.hasNext();) {
                    try {
                        ...
                        Object filterValue = filters.get(filterProperty);
                           ...
                           try {
                               field = Drzava.class.getDeclaredField(filterProperty);      
